...
def plot_stats(stats, directory='figs'):
    plt.clf()
    plt.title('Diversity Through Training (L2 from Uniform)')
    plt.xlabel('Epoch')
    plt.ylabel('Normalized L2 Distance From Uniform Distribution')
    plt.plot(stats['diversity'])
    plt.savefig(directory + '/diversity.png')

    plt.clf()
    plt.title('Avg. Log Likelihood of GAN Samples wrt Target')
    plt.xlabel('Epoch')
    plt.ylabel('Log Likelihood')
    plt.plot(stats['likelihood'])
    plt.savefig(directory + '/likelihood.png')
...
def make_grid_data(n_gauss=3, std=0.01):
    ...
    gmm = sklearn.mixture.GaussianMixture(n_components=n_gauss*n_gauss).fit(dataset)
...
for it in range(30000):
    ...
    if (it+1) % plot_every == 0:
        samples = sess.run(G_sample, feed_dict={Z: sample_Z(5000, Z_dim)})
        likelihood = -gmm.score(samples)
        stats['likelihood'].append(likelihood)
        
        hist = np.histogram(gmm.predict(samples), bins=range(10))
        
        target_hist = np.ones(9)*samples.shape[0]/9.
        # Now find the L2 Distance between this and a uniform histogram
        l2_normalized = np.linalg.norm(hist[0]-target_hist)/len(samples)
        stats['diversity'].append(l2_normalized)
        print(stats)
        # Clear plot
        plt.clf()     
        # Fit GMM to output
        ...
        plot_results(samples, dpgmm.predict(samples), dpgmm.means_, dpgmm.covariances_, it,
          'Iter: {}, loss(D): {:2.2f}, loss(G):{:2.2f}, \n likelihood:{:2.2f}, diversity:{:2.2f}'.format(
            it+1, D_loss_curr, G_loss_curr, likelihood, l2_normalized))
